...
IF EXISTS (SELECT * FROM [sys].[identity_columns] WHERE [name] IN (N'ComputerSpecId', N'ComputerSpecComponentId', N'Count') AND [object_id] = OBJECT_ID(N'[ComputerSpecComponent]'))
    SET IDENTITY_INSERT [ComputerSpecComponent] ON;
INSERT INTO [ComputerSpecComponent] ([ComputerSpecId], [ComputerSpecComponentId], [Count])
VALUES (1, 1, 1);
IF EXISTS (SELECT * FROM [sys].[identity_columns] WHERE [name] IN (N'ComputerSpecId', N'ComputerSpecComponentId', N'Count') AND [object_id] = OBJECT_ID(N'[ComputerSpecComponent]'))
...
VALUES (N'20240204011149_InitialCreate', N'8.0.1');
GO
