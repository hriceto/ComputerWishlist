...
IF EXISTS (SELECT * FROM [sys].[identity_columns] WHERE [name] IN (N'ComputerSpecId', N'ComputerSpecComponentId') AND [object_id] = OBJECT_ID(N'[ComputerSpecComponent]'))
    SET IDENTITY_INSERT [ComputerSpecComponent] ON;
INSERT INTO [ComputerSpecComponent] ([ComputerSpecId], [ComputerSpecComponentId])
VALUES (1, 1);
IF EXISTS (SELECT * FROM [sys].[identity_columns] WHERE [name] IN (N'ComputerSpecId', N'ComputerSpecComponentId') AND [object_id] = OBJECT_ID(N'[ComputerSpecComponent]'))
